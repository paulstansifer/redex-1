...
A @deftech{binding pattern} is like a @pattern, except that inside it, any @tech{binding 
pattern} may be followed by @racket[#:refers-to beta]. A @deftech{beta} is a tree of 
references to names bound by the @tech{binding pattern}. (Note that the ``binding'' of
pattern variables by Redex patterns is otherwise unrelated to the ``binding'' of symbols
inside Redex terms that we discuss here.) 
...
@subsection{The structure of binding forms}
...
@subsection{Behavior of binding forms}
