...
@(racketgrammar*
   ...
   [beta binding:nothing
         pat:symbol
         (binding:rib beta-sequence ...)
         (binding:shadow beta-sequence ...)]
   [beta-sequence beta
...
@racket[shadow] (later names override previous names) or @racket[rib] (names must be distinct):
...
@racket[subform-export-beta] (here, @racket[(shadow clauses x_v)]) to the preceding repetition or
the outer binding form, both of which can refer to it by @racket[subform-name] (here,
...
for). Furthermore, since each clause exports @racket[(shadow clauses x_v)], the last clause exports
@racket[c], the second-to-last clause exports @racket[c] and @racket[b], and the first one exports
all three, which are then referred-to by the body of the @racket[let*].
...
outside of it, or vice versa. For example, @racket[e_body #:refers-to (shadow x_v ...)] would have
been illegal in the above example.
